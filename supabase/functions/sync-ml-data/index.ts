...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Date cutoff: last 12 months to stay within Edge Function timeout
    ...
    if (sync_type === 'all' || sync_type === 'po_delivery') {
      ...
      try {
        const purchaseOrders = await queryOdoo('purchase.order', 'search_read', [
          [['state', 'in', ['purchase', 'done']], ['date_order', '>=', cutoffStr]],
    ...
    if (sync_type === 'all' || sync_type === 'production') {
      ...
      try {
        const manufacturingOrders = await queryOdoo('mrp.production', 'search_read', [
          [['state', 'in', ['done', 'progress', 'confirmed']], ['date_planned_start', '>=', cutoffStr]],
    ...
    if (sync_type === 'all' || sync_type === 'supplier_product_metrics') {
      ...
      try {
        const confirmedPOs = await queryOdoo('purchase.order', 'search_read', [
          [['state', 'in', ['purchase', 'done']], ['date_order', '>=', cutoffStr]],
          ['id', 'name', 'partner_id', 'date_order'],
        ]);

        if (Array.isArray(confirmedPOs) && confirmedPOs.length > 0) {
          const poIds = confirmedPOs.map((po: any) => po.id);
          const poLookup = new Map<number, any>();
          for (const po of confirmedPOs) { poLookup.set(po.id, po); }

          const allLines = await queryOdoo('purchase.order.line', 'search_read', [
            [['order_id', 'in', poIds]],
            ['order_id', 'product_id', 'product_qty', 'price_unit', 'price_subtotal', 'date_planned'],
          ]);

          // Fetch stock.picking for actual receipt dates
          const poNames = confirmedPOs.map((po: any) => po.name);
          let pickings: any[] = [];
          try {
            pickings = await queryOdoo('stock.picking', 'search_read', [
              [['origin', 'in', poNames], ['state', '=', 'done'], ['picking_type_code', '=', 'incoming']],
              ['origin', 'date_done', 'scheduled_date'],
            ]);
            if (!Array.isArray(pickings)) pickings = [];
          } catch (e) {
            console.warn('stock.picking incoming query failed:', e);
          }
          ...
          if (Array.isArray(allLines) && allLines.length > 0) {
            ...
            for (const line of allLines) {
              const poId = Array.isArray(line.order_id) ? line.order_id[0] : line.order_id;
              const po = poLookup.get(poId);
              if (!po) continue;

              const vendorName = Array.isArray(po.partner_id) ? po.partner_id[1] : String(po.partner_id || '');
              const productId = Array.isArray(line.product_id) ? String(line.product_id[0]) : String(line.product_id || '');
              const productName = Array.isArray(line.product_id) ? line.product_id[1] : '';
              ...
              // Actual receipt date from stock.picking
              const poName = Array.isArray(po.name) ? po.name : String(po.name || '');
              const picking = pickingByOrigin.get(poName);
              const actualDate = picking?.date_done?.split(' ')[0] || null;
            ...
            for (const [, stats] of vpMap.entries()) {
              const avgLT = stats.lead_times.length > 0
                ? stats.lead_times.reduce((a: number, b: number) => a + b, 0) / stats.lead_times.length : 0;
              const ltStddev = stats.lead_times.length > 1
                ? Math.sqrt(stats.lead_times.reduce((sum: number, lt: number) => sum + Math.pow(lt - avgLT, 2), 0) / (stats.lead_times.length - 1)) : 0;
              const onTimeRate = stats.total_orders > 0 ? stats.on_time_count / stats.total_orders : 0;
              const avgDelay = stats.delays.length > 0
                ? stats.delays.reduce((a: number, b: number) => a + b, 0) / stats.delays.length : 0;
              const avgPrice = stats.prices.length > 0
                ? stats.prices.reduce((s: number, p: any) => s + p.price, 0) / stats.prices.length : 0;
              ...
              upsertBatch.push({
                vendor_name: stats.vendor_name, product_id: stats.product_id, product_name: stats.product_name,
                avg_lead_time: Math.round(avgLT * 10) / 10, lead_time_stddev: Math.round(ltStddev * 10) / 10,
                on_time_rate: Math.round(onTimeRate * 1000) / 1000, avg_delay_days: Math.round(avgDelay * 10) / 10,
                avg_unit_price: Math.round(avgPrice * 100) / 100, price_trend_pct: Math.round(priceTrend * 10) / 10,
                total_orders: stats.total_orders, total_qty: stats.total_qty,
                total_spend: Math.round(stats.total_spend * 100) / 100,
                last_order_date: stats.last_order_date, updated_at: new Date().toISOString(),
              });
            }
            ...
            results.supplier_product_metrics = { synced: 0, total: 0, note: 'No PO lines found' };
          }
        } else {
          ...
          for (const q of quants) {
            ...
            if (!invMap.has(key)) {
              invMap.set(key, {
                ...
                warehouse_name: warehouse, qty_on_hand: 0, qty_reserved: 0,
              });
            }
      ...
      try {
        ...
        if (Array.isArray(orderpoints) && orderpoints.length > 0) {
          ...
          for (const op of orderpoints) {
            ...
            upsertBatch.push({
              product_id: productId, product_name: productName, warehouse_name: warehouseName,
              odoo_orderpoint_id: op.id,
              odoo_min_qty: op.product_min_qty || 0, odoo_max_qty: op.product_max_qty || 0,
              odoo_qty_multiple: op.qty_multiple || 1, odoo_lead_days: op.lead_days_date || 0,
